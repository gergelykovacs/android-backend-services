...
public class NetworkingStrategyFactory {
	
	private NetworkingStrategyFactory() {
	}
	
	public static IStrategy getStrategy(ServiceEvent event, List<ServiceData> data, IServiceConnection serviceConnection, Context context) {
		...
		switch(event) {
			...
			case EVENT_SIGN_IN:
			case EVENT_SIGN_OUT:
				strategy = new UserNetworkingStrategy(event, data, serviceConnection, context);
				break;
			case EVENT_GET_QUESTIONS:
			case EVENT_ADD_QUESTION:
			case EVENT_DELETE_QUESTION:
				strategy = new QuestionNetworkingStrategy(event, data, serviceConnection, context);
				break;
			case EVENT_GET_ANSWERS:
			case EVENT_ADD_ANSWER:
			case EVENT_DELETE_ANSWER:
				strategy = new AnswerNetworkingStrategy(event, data, serviceConnection, context);
				break;
			default:
